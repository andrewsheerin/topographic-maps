...
| Date | Decision | Why |
| --- | --- | --- |
| 2026-08-28 | F-11 place picker uses TIGER **county subdivisions** (CB 2023 1:500k), not TIGER Places | Mirrors swpt-app D-28 — the user's own prior call there (Places leave unincorporated holes); flagged for override in the session summary |
| 2026-08-28 | Ported swpt-app's bbox `area_km2` display approximation (111.320/110.574 km/deg) verbatim | Display-only picker metadata, already in use in the user's swpt-app; no scientific output touched |
| 2026-08-28 | Renamed upload endpoint to `/api/upload-boundary` (was `/api/upload-shapefile`) | It now accepts GeoJSON too; our frontend is the only caller |
| 2026-08-28 | Places API: missing dataset → HTTP 503 with the exact fetch command (swpt returns `[]`) | L1 "errors say what to do" — an empty picker with no explanation is a dead end |
| 2026-08-28 | TIGER data at `data/tiger/subdivisions.gpkg`, gitignored, regenerated by `backend/scripts/fetch_tiger_subdivisions.py` | Matches repo data rules: script committed, generated output not |
| 2026-08-28 | PlacePicker owns its list-search fetching; Generator fetches the chosen boundary | Keeps picker-internal state out of Generator; the app-state mutation stays at page level |
| 2026-08-28 | Multi-part place boundaries (island towns) reduce to convex hull, same as uploads | `core/roads.py` needs one exterior ring; map then shows exactly what gets meshed. Revisit if hull-over-water bothers |
